...
def download_file(bucket_name,object_key,local_path):
    ...
    try:
        log_info(f"Downloading | {object_key} from {bucket_name}")
        s3 = get_s3_client()
        s3.download_file(bucket_name,object_key,local_path)
        log_info(f"Download Successful | Saved to {local_path}")
    except Exception as e:
...
def read_csv(input_local_path):
    ...
    log_info(f"Reading File | {input_local_path.split('/')[-1]}")
    row_list = []
    try:
        ...
        log_info(f"Reading Successful")
    except Exception as e:
        log_error(f"Reading Failed: {e}")
        raise
